module ACCUMULATOR_REGISTER #(
	...
	reg [REG_WIDTH - 1 : 0] acc; // actual acc register
	
	wire [REG_WIDTH - 1 : 0] mux1 = load ? globalOut : ALUOut;
	wire [REG_WIDTH - 1 : 0] data = ReadJalRegisters ? JALRegOut : mux1;
	
	always @(posedge clk) begin // register clock logic
		if(rst)
			acc <= {REG_WIDTH{1'b0}}; // all 0's
