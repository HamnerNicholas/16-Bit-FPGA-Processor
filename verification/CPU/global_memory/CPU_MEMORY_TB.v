...
module CPU_MEMORY_TB;
    ...
    begin
        ...
        if (dut.global_memory_inst.global_mem[address] !== expected) begin
            tests_failed = tests_failed + 1;

            $display(
                "FAIL - Test %0d: memory[0x%0h] expected 0x%0h, actual 0x%0h",
                test_number,
                address,
                expected,
                dut.global_memory_inst.global_mem[address]
            );
        end
        else begin
            $display(
                "PASS - Test %0d: memory[0x%0h] = 0x%0h",
                test_number,
                address,
                dut.global_memory_inst.global_mem[address]
            );
...
endtask
    ...
    initial begin
        ...
        $display("");
        $display("========================================");
        $display("CPU GLOBAL MEMORY TEST");
        $display("========================================");
        ...
        /*
         * Verify that halt freezes processor state and memory.
         */
        repeat (3) begin
            @(posedge clk);
            #1;
        ...
        $display("MEMORY[0x10]: 0x%0h", dut.global_memory_inst.global_mem[8'h10]);
        $display("HALT:         %b", halt);
        $display("========================================");
